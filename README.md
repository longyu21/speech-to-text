...
- URL 转写失败后可保留已完成片段，重试时从上次失败位置继续处理
- URL 翻译支持按片段显示进度，可暂停后继续，不再只是取消当前请求
- 支持单文件和批量文件入队转写
- 支持队列式异步转写、失败重试和文件删除
- 下方结果区展示当前转写文本和历史记录
- 允许下载 `.docx` Word 文档
...
- 创建上传目录和生成语音目录
- 自动恢复本地上传转写任务
- 将服务重启前未完成的 URL 转写任务标记为暂停，需手动点击“重试”后继续
...
5. URL 转写完成后可切换翻译语言、删除最近履历，也可以点击某个时间片段快速跳转播放位置；翻译会按片段显示实时进度，并支持暂停后继续。
